...
TEST_CASE( "resample_audio_48000_to_41000", "[libopenshot][framemapper]" )
{
	...
	CHECK(map.GetFrame(1)->GetAudioSamplesCount() == Catch::Detail::Approx(882).margin(10.0));
	CHECK(map.GetFrame(2)->GetAudioSamplesCount() == Catch::Detail::Approx(882).margin(10.0));
	CHECK(map.GetFrame(50)->GetAudioSamplesCount() == Catch::Detail::Approx(882).margin(10.0));
	...
	for (auto& rate : arr) {
		...
		for (int frame_index = 1; frame_index <= map.info.fps.ToInt(); frame_index++) {
			...
			for (int sample_index = 0; sample_index < sample_count; sample_index++) {
				...
				CHECK(resampled_value == Catch::Detail::Approx(sample_value).margin(0.1));
			}
		...
		for (int frame_index = 1; frame_index < 24; frame_index++) {
			...
			for (int i = 0; i < sample_count; i++) {
				...
				CHECK(resampled_value == Catch::Detail::Approx(sample_value * 2.0).margin(0.1));
		...
		for (auto &mapped_rate : mapped_rates) {
			...
			for (int frame_index = 1; frame_index <= (map.info.fps.ToInt() * num_seconds); frame_index++) {
				...
				for (int sample_index = 0; sample_index < sample_count; sample_index++) {
					...
					CHECK(predicted_value == Catch::Detail::Approx(mapped_value).margin(0.001));
				}
			}
			...
			for (int frame_index = starting_clip_frame; frame_index < (starting_clip_frame + (t1.info.fps.ToFloat() * num_seconds)); frame_index++) {
				for (int sample_index = 0; sample_index < t1.GetFrame(frame_index)->GetAudioSamplesCount(); sample_index++) {
					...
					CHECK(predicted_value == Catch::Detail::Approx(timeline_value).margin(0.001));
				}
			}
